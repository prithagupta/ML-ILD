...
def optimize_search_cv(classifier, params, search_space, cv_iterator, hp_iterations, x, y, val_metric='accuracy',
    ...
    if 'n_jobs' in params.keys() and classifier.__name__ != LogisticRegression.__name__:
        if 'PFS_FOLDER' in os.environ:
            ...
            params['n_jobs'] = os.cpu_count() - inner_cv_iterator.n_splits - 3
...
def optimize_search_cv_known(classifier, params, cv_iterator, x, y, best_params_, random_state=42):
    logger = logging.getLogger('Classifier-Test')
    # clf = classifier(**params)
    # y_pred = None
    # pred_prob = None
    ...
    logger.info("Current default parameters {}".format(print_dictionary(params, sep='\t')))
    scores = dict()
    best_params_ = np.array(best_params_)
    for i, (train_index, test_index) in enumerate(cv_iterator.split(x, y)):
        logger.info("####################### Starting the iteration {} #######################".format(i + 1))
        ...
        params.update(best_params_[i][1])
        params_str = print_dictionary(best_params_[i][1], sep='\t')
        logger.info("For the outer split {} Best parameters are: {} with accuracy "
                    "of: {:.4f}\n".format(i + 1, params_str, best_params_[i][0]))
        if best_params_[i][0] < 0.5:
            idx = np.argmax(np.array(best_params_[:, 0], dtype=float))
            bp = best_params_[idx][1]
            score = best_params_[idx][0]
            params.update(bp)
            params_str = print_dictionary(bp, sep='\t')
            logger.info("Updating to best parameters Best parameters are: {} with accuracy "
                        "of: {}\n".format(params_str, score))
        ...
        informedness = get_evaluation(informedness, instance_informedness, y_test, y_pred, logger, i)
        best_parameters.append([accuracies[i], copy.deepcopy(params)])
    idx = np.argmax(np.array(accuracies, dtype=float))
    for i in range(cv_iterator.n_splits):
        if accuracies[i] < 0.5:
            confusion_matrices[i] = confusion_matrices[idx]
            f1_scores[i] = f1_scores[idx]
            accuracies[i] = accuracies[idx]
            cohen_kappa[i] = cohen_kappa[idx]
            aucs[i] = aucs[idx]
            mccs[i] = mccs[idx]
            informedness[i] = informedness[idx]
            best_parameters[i] = best_parameters[idx]
